...
Open any `.xaml` file to use the visual XAML Designer. The designer provides a searchable toolbox, drag-and-drop into layout containers, selection and resizing, Canvas movement, grid snapping, zoom, and an embedded property grid. Visual changes update the XAML text document and participate in VS Code undo/redo.

Use **WinDev: Open XAML as Text** or the `</>` toolbar button to switch to source. Press `Ctrl+Shift+V` to reopen the current XAML file in the designer.

The editable HTML/CSS canvas works on Windows, macOS, and Linux. On Windows, choose **Preview** in the toolbar to verify the current XAML with the native WinUI renderer. See the [XAML Designer guide](xaml-designer.md) for layout behavior, event generation, settings, and limitations.
